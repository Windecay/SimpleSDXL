...
def refresh_output_list(max_per_page, max_catalog, user_did=None, engine_type='image'):
    ...
    for index in listdirs:
        ...
        video_files.update({"{}{}{}".format(index[-5:-3],index[-2:],''.join(v.split('_')[1].split('-')) if '_' in v else ''): os.path.join(index, v) for v in util.get_files_from_folder(path_gallery, video_types, None)})
