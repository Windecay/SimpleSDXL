...
export class CanvasLayers {
    ...
    showBlendModeMenu(worldX, worldY) {
        ...
        titleText.textContent = `混合模式: ${selectedLayer.name}`;
        ...
        blendAreaLabel.textContent = '混合区域';
    ...
    async fuseLayers() {
        if (this.canvas.canvasSelection.selectedLayers.length < 2) {
            showErrorNotification("请至少选择2个图层进行合并。");
            return;
        }
        ...
        try {
            ...
            if (fusedWidth <= 0 || fusedHeight <= 0) {
                ...
                showErrorNotification("无法合并图层：计算出的尺寸无效。");
                return;
            }
            ...
            showErrorNotification(`合并图层时出错: ${error.message}`);
        }
    }
}
