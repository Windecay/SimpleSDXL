...
export class CanvasIO {
    ...
    async _performSave(fileName, outputMode) {
        // Allow saving empty canvas (e.g. when all layers are deleted)
        if (this.canvas.layers.length === 0) {
            log.info(`Node ${this.canvas.node.id} has no layers, saving empty state`);
        }
    ...
    async checkForInputData(options) {
        try {
            ...
            // In Gradio environment (iframe), use postMessage to request input data
            if (window.location.search.includes("api_url")) {
                log.info("Running in Gradio iframe, requesting input data via postMessage");
    ...
    async importLatestImage() {
        log.info("[DEBUG] Manual import requested.");
        try {
            // Check if running in iframe (Gradio mode) - More robust check
            const inIframe = window.self !== window.top;
            const hasApiUrl = window.location.search.includes("api_url");
            
            log.info(`[DEBUG] Environment check: inIframe=${inIframe}, hasApiUrl=${hasApiUrl}, href=${window.location.href}`);

            if (inIframe || hasApiUrl) {
                log.info(`[DEBUG] Running in iframe, sending REQUEST_INPUT message`);
                window.parent.postMessage({ type: 'REQUEST_INPUT' }, '*');
                return;
            }
            
            log.info("[DEBUG] Not in iframe, proceeding with backend check");
            await this.checkForInputData('manual_import', true, true);
        } catch (error) {
            log.error("[DEBUG] Error importing latest image:", error);
            showErrorNotification("导入输入失败: " + error.message);
        }
    }
    scheduleDataCheck() {
    ...
    async importLatestImage() {
        try {
            log.info("Manually importing input image...");
            ...
            if (!hasImageInput && !hasMaskInput) {
                showErrorNotification("未连接输入图像或遮罩节点");
                return false;
            }

            // Force reset cache to allow reloading
            this.canvas.lastLoadedImageSrc = undefined;
            this.canvas.lastLoadedLinkId = undefined;
            this.canvas.inputDataLoaded = false;

            // Trigger input data check with explicit manual_import reason
            await this.checkForInputData({ reason: 'manual_import', allowImage: true, allowMask: true });
            
            showSuccessNotification("已导入输入图像");
            return true;
        }
        catch (error) {
            log.error("Error importing input image:", error);
            showErrorNotification(`导入输入图像失败: ${error.message}`);
            return false;
        }
    }
    async importLatestImages(sinceTimestamp, targetArea = null) {
        try {
            ...
            showErrorNotification(`导入最新图像失败: ${error.message}`);
            return [];
        }
    }
