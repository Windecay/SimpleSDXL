...
import { showInfoNotification, showSuccessNotification, showErrorNotification } from "/file=javascript/layerforge/js/utils/NotificationUtils.js?v=patch25";
import { uploadCanvasAsImage, uploadImageBlob } from "/file=javascript/layerforge/js/utils/ImageUploadUtils.js?v=patch25";
...
import { validateAndFixClipspace } from "/file=javascript/layerforge/js/utils/ClipspaceUtils.js?v=patch25";
...
function handleSAMDetectorModalClosed(node) {
    ...
    if (node.imgs && node.imgs.length > 0) {
        ...
        if (currentImgSrc && currentImgSrc !== originalImgSrc) {
            ...
            showInfoNotification("SAM 检测器已关闭。未应用遮罩。");
        }
    }
    else {
...
async function handleSAMDetectorResult(node, resultImage) {
    try {
        ...
        showSuccessNotification("SAM 检测器遮罩已应用到 LayerForge！");
    }
    catch (error) {
        log.error("Error processing SAM Detector result:", error);
        // Show error notification
        showErrorNotification(`应用 SAM 遮罩失败: ${error.message}`);
    }
    finally {
        node.samMonitoringActive = false;
