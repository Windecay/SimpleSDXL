...
export function showNotification(message, backgroundColor = "#4a6cd4", duration = 3000, type = "info", deduplicate = false) {
    ...
    const config = {
        success: { icon: "✔️", title: "成功", bg: "#1fd18b" },
        error: { icon: "❌", title: "错误", bg: "#ff6f6f" },
        info: { icon: "ℹ️", title: "信息", bg: "#4a6cd4" },
        warning: { icon: "⚠️", title: "警告", bg: "#ffd43b" },
        alert: { icon: "⚠️", title: "提示", bg: "#fff7cc" }
    }[type];
    ...
    if (type === 'error') {
        ...
        copyButton.onclick = () => {
            navigator.clipboard.writeText(message)
                .then(() => showSuccessNotification("错误信息已复制！", 2000))
