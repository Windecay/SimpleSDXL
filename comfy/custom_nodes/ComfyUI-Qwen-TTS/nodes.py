...
QWEN_TTS_DIRECT_MODEL_URLS: Dict[str, List[Tuple[str, str]]] = {}

_MODELSCOPE_KNOWN_FILES: Dict[str, List[str]] = {
...
_SPEECH_TOKENIZER_REPO_ID = "Qwen/Qwen3-TTS-Tokenizer-12Hz"
_SPEECH_TOKENIZER_PREFIX = "speech_tokenizer/"

def _modelscope_resolve_url(model_id: str, rel_path: str) -> str:
    if rel_path.startswith(_SPEECH_TOKENIZER_PREFIX):
        rel_path = rel_path[len(_SPEECH_TOKENIZER_PREFIX):]
        model_id = _SPEECH_TOKENIZER_REPO_ID
    return f"https://www.modelscope.cn/models/{model_id}/resolve/master/{rel_path}"

def _try_scrape_modelscope_files(model_id: str) -> Optional[List[str]]:
    ...
    names = []
    for m in re.finditer(r"下载\s*([A-Za-z0-9][A-Za-z0-9._\-]+)", text):
...
def _autofill_direct_urls_for_model(model_id: str) -> None:
    if model_id in QWEN_TTS_DIRECT_MODEL_URLS:
        return
    file_list = _MODELSCOPE_KNOWN_FILES.get(model_id)
    if not file_list:
        file_list = _try_scrape_modelscope_files(model_id)
    if not file_list:
        return
    QWEN_TTS_DIRECT_MODEL_URLS[model_id] = [(_modelscope_resolve_url(model_id, n), n) for n in file_list]
...
def download_model_if_needed(model_id: str, qwen_root: str, target_dir_override: Optional[str] = None) -> str:
    """Download a specific model if not found locally"""
    folder_name = model_id.split("/")[-1]
    target_dir = target_dir_override or os.path.join(qwen_root, folder_name)
    
    def _get_remote_total(url: str) -> Optional[int]:
        import urllib.request
        try:
    ...
    def _is_up_to_date(url: str, dst_path: str) -> bool:
        if not (os.path.exists(dst_path) and os.path.isfile(dst_path)):
            return False
        total = _get_remote_total(url)
        if total is None:
            return True
        try:
            return os.path.getsize(dst_path) == total
        except Exception:
            return False

    def _download_file(url: str, dst_path: str, rel_path: str) -> None:
        import urllib.request
        import time

        os.makedirs(os.path.dirname(dst_path), exist_ok=True)
        rel_name = rel_path or (os.path.basename(dst_path) or dst_path)
        try:
            with urllib.request.urlopen(url) as r:
                total = None
                try:
                ...
                if os.path.exists(dst_path) and os.path.isfile(dst_path) and total is not None:
                    try:
                        if os.path.getsize(dst_path) == total:
                            return
                    except Exception:
                        pass
                ...
                t0 = time.time()
                next_pct = 0.0
                print(f"⬇️  [Qwen3-TTS] {rel_name} start" + (f" ({total / (1024 * 1024):.2f} MB)" if total else ""))
                    ...
                    while True:
                        ...
                        if total:
                            ...
                            if pct + 1e-9 >= next_pct:
                                ...
                                print(
                                    f"   [Qwen3-TTS] {rel_name}: {downloaded / (1024 * 1024):.2f}/{total / (1024 * 1024):.2f} MB "
                                    f"({pct:.1f}%) {avg_speed / (1024 * 1024):.2f} MB/s"
                                )
                                next_pct = (int(pct / 5.0) + 1) * 5.0
                ...
                elapsed = max(time.time() - t0, 1e-6)
                avg_speed = downloaded / elapsed
                print(f"✅ [Qwen3-TTS] {rel_name} done ({downloaded / (1024 * 1024):.2f} MB, {avg_speed / (1024 * 1024):.2f} MB/s)")
        finally:
            try:
                tmp_path = dst_path + ".part"
    ...
    _autofill_direct_urls_for_model(model_id)
    items = QWEN_TTS_DIRECT_MODEL_URLS.get(model_id)
    if not items:
        if os.path.exists(target_dir) and os.path.isdir(target_dir):
            return target_dir
        raise RuntimeError(
            f"[Qwen3-TTS] Model not found locally: {model_id}. "
            f"Expected directory: {target_dir}. "
            f"Please pre-download the model files into that directory, or configure QWEN_TTS_DIRECT_MODEL_URLS."
        )
    ...
    for url, rel_path in items:
        ...
        if not _is_up_to_date(url, dst_path):
            pending.append((url, rel_path, dst_path))

    if not pending:
        return target_dir

    print(f"\n📥 [Qwen3-TTS] Downloading {model_id} (direct URLs)...")
    total_items = len(pending)
    for idx, (url, rel_path, dst_path) in enumerate(pending, start=1):
        print(f"   [Qwen3-TTS] ({idx}/{total_items}) {rel_path}")
        print(f"      [Qwen3-TTS] url:  {url}")
        print(f"      [Qwen3-TTS] save: {dst_path}")
        _download_file(url, dst_path, rel_path)
...
def check_and_download_tokenizer():
    ...
    qwen_roots = []
    try:
        qwen_roots = folder_paths.get_folder_paths("qwen-tts") or []
    except Exception:
        qwen_roots = []
    qwen_root = qwen_roots[0] if qwen_roots else os.path.join(folder_paths.models_dir, "qwen-tts")
...
def load_qwen_model(model_type: str, model_choice: str, device: str, precision: str, attention: str = "auto", unload_after: bool = False, previous_attention: str = None, custom_model_path: Optional[str] = None):
    ...
    model_id = final_source
    if found_local:
        final_source = found_local
        print(f"✅ [Qwen3-TTS] Loading local model: {os.path.basename(final_source)}")
        if not (custom_model_path and isinstance(custom_model_path, str) and custom_model_path.strip()):
            qwen_roots = []
            try:
                qwen_roots = folder_paths.get_folder_paths("qwen-tts") or []
            except Exception:
                qwen_roots = []
            qwen_root = qwen_roots[0] if qwen_roots else os.path.join(folder_paths.models_dir, "qwen-tts")
            download_model_if_needed(model_id, qwen_root, target_dir_override=final_source)
    else:
        qwen_roots = []
        try:
            qwen_roots = folder_paths.get_folder_paths("qwen-tts") or []
