...
sam_model_dir_name = ["inpaint", "sams"]
sam_model_list = {
    "sam_vit_h (2.56GB)": {
...
groundingdino_model_dir_name = ["inpaint", "grounding-dino"]
...
def get_local_filepath(url, dirname, local_file_name=None):
    ...
    dirnames = dirname if isinstance(dirname, (list, tuple, set)) else [dirname]
    for dirname_candidate in dirnames:
        destination = folder_paths.get_full_path(dirname_candidate, local_file_name)
        if destination:
            logger.warn(f'using extra model: {destination}')
            return destination

        destination = os.path.join(folder_paths.models_dir, dirname_candidate, local_file_name)
        if os.path.isfile(destination):
            logger.warn(f'using extra model: {destination}')
            return destination

    folder = os.path.join(folder_paths.models_dir, dirnames[0])
