...
export class LayerForgeMaskEditor {
    ...
    updateTransform() {
        ...
        // Ensure canvases fill the container
    ...
    drawDot(x, y) {
        ...
        if (this.isEraser) {
            ...
            if (this.brushHardness >= 0.99) {
                ...
                // Soft eraser using gradient
                const gradient = this.maskCtx.createRadialGradient(x, y, 0, x, y, radius);
                // Calculate stop position for hardness
                // Hardness 1.0 -> 100% solid
                // Hardness 0.0 -> 0% solid (all fade)
        ...
        this.maskCtx.restore(); // Restore context state
    }
    ...
    draw(x, y) {
        ...
        // Interpolate dots for smooth stroke
        // Step size proportional to brush size but small enough for smoothness
