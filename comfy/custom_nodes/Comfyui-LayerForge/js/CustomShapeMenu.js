...
export class CustomShapeMenu {
    ...
    _createUI() {
        ...
        minimizedBar.textContent = "自定义输出区域激活";
        ...
        const checkboxContainer = this._createCheckbox('auto-apply-checkbox', () => this.canvas.autoApplyShapeMask, '自动应用形状遮罩', (e) => {
            ...
            this._updateUI();
            this.canvas.render();
        }, "根据自定义输出区域形状自动应用遮罩。启用后，遮罩将应用于形状边界内的所有图层。");
        featureContainer.appendChild(checkboxContainer);
        // Add expansion checkbox
        const expansionContainer = this._createCheckbox('expansion-checkbox', () => this.canvas.shapeMaskExpansion, '扩展/收缩遮罩', (e) => {
        ...
        }, "扩张（扩展）或侵蚀（收缩）形状遮罩。正值向外扩展遮罩，负值向内收缩。");
        ...
        expansionSliderLabel.textContent = '扩展量：';
        ...
        const featherContainer = this._createCheckbox('feather-checkbox', () => this.canvas.shapeMaskFeather, '羽化边缘', (e) => {
        ...
        featherSliderLabel.textContent = '羽化程度:';
        ...
        slidersContainer.appendChild(createExtensionSlider('顶部扩展:', 'top'));
        slidersContainer.appendChild(createExtensionSlider('底部扩展:', 'bottom'));
        slidersContainer.appendChild(createExtensionSlider('左侧扩展:', 'left'));
        slidersContainer.appendChild(createExtensionSlider('右侧扩展:', 'right'));
